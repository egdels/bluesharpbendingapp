...
/**
 * Test class for detecting 2-note and 3-note chords on a Richter harmonica. This class tests the
 * ability to detect adjacent blown and drawn notes on a Richter harmonica with a tolerance of -50
 * to 50 cents.
 */
class RichterHarmonicaChordTest {
  ...
  void testAdjacentBlownNotes(int hole1, int hole2) {
    ...
    double[] chord =
        generateChord(new double[] {freq1, freq2}, new double[] {1.0, 1.0}, SAMPLE_RATE, 1.0);
  ...
  void testThreeAdjacentBlownNotes(int hole1, int hole2, int hole3) {
    ...
    double[] chord =
        generateChord(
            new double[] {freq1, freq2, freq3}, new double[] {1.0, 1.0, 1.0}, SAMPLE_RATE, 1.0);
    ...
    assertTrue(
        foundFreq1 || foundFreq2 || foundFreq3,
   ...
   * is covered by the tongue on a Richter harmonica. Each test case represents three adjacent holes
   * where the middle hole is covered, resulting in a chord of the outer two holes.
   */
  @ParameterizedTest
  @CsvSource({
  ...
  void testThreeAdjacentBlownNotesWithMiddleCovered(int hole1, int hole2, int hole3) {
    ...
    double[] chord =
        generateChord(new double[] {freq1, freq3}, new double[] {1.0, 1.0}, SAMPLE_RATE, 1.0);
  ...
  /**
   * Tests the detection of 2-note chords formed when the middle two holes of four adjacent blown
   * holes are covered by the tongue on a Richter harmonica. Each test case represents four adjacent
   * holes where the middle two holes are covered, resulting in a chord of the outer two holes.
   */
  @ParameterizedTest
  @CsvSource({
  ...
  void testFourAdjacentBlownNotesWithMiddleTwoCovered(int hole1, int hole2, int hole3, int hole4) {
    ...
    double[] chord =
        generateChord(new double[] {freq1, freq4}, new double[] {1.0, 1.0}, SAMPLE_RATE, 1.0);
  ...
  /**
   * Tests the detection of 2-note chords formed when the middle two holes of four adjacent drawn
   * holes are covered by the tongue on a Richter harmonica. Each test case represents four adjacent
   * holes where the middle two holes are covered, resulting in a chord of the outer two holes.
   */
  @ParameterizedTest
  ...
  /**
   * Tests that 5-note chords on a Richter harmonica are NOT recognized as valid chords. This test
   * verifies that 5-note chords, which don't make sense on a harmonica, are not recognized as valid
   * harmonica chords.
   */
  @ParameterizedTest
  @CsvSource({
  ...
  void testFiveNoteChords(int hole1, int hole2, int hole3, int hole4, int hole5, int noteType) {
    ...
    // However, 5-note chords don't make sense on a harmonica because they don't form a valid
    // harmonica chord pattern
  ...
  /**
   * Tests that non-valid hole combinations on a Richter harmonica are NOT recognized as valid
   * chords. This test verifies that combinations of holes that are not part of the valid harmonica
   * chord patterns (adjacent, 3 adjacent with middle covered, or 4 adjacent with middle two
   * covered) are not recognized as valid harmonica chords.
   */
  @ParameterizedTest
  @CsvSource({
