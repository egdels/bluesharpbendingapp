...
/**
 * Test class for the ChordDetector. This class tests the functionality of the chord detection
 * algorithm.
 */
class ChordDetectorTest {
  ...
  @ParameterizedTest
  @CsvSource({
    // First frequency, first amplitude, second frequency, second amplitude, third frequency, third
    // amplitude
  ...
  void testChordDetection(
      double freq1, double amp1, double freq2, double amp2, double freq3, double amp3) {
    // Generate a chord (multiple sine waves)
    double[] chord =
        generateChord(
            new double[] {freq1, freq2, freq3}, new double[] {amp1, amp2, amp3}, SAMPLE_RATE, 1.0);
    ...
    assertTrue(
        result.getPitchCount() == 3,
        "Should detect 3 pitches in the chord but was " + result.getPitchCount());
  ...
  /**
   * Tests the chord detector's ability to identify two-note harmonic chords that are an octave
   * apart.
   *
  ...
  void testTwoNoteHarmonicaChordDetectionOctave(
      double freq1, double amp1, double freq2, double amp2) {
    // Generate a 2-note chord (two sine waves)
    double[] chord =
        generateChord(new double[] {freq1, freq2}, new double[] {amp1, amp2}, SAMPLE_RATE, 1.0);
  ...
  @CsvSource({
    // First frequency, first amplitude, second frequency, second amplitude, third frequency, third
    // amplitude, fourth frequency, fourth amplitude
    ...
    "246.94, 1.0, 311.13, 1.0, 369.99, 1.0, 493.88, 1.0", // B diminished 7th chord (B3, D4, F4,
    // Ab4)
  ...
  void testFourNoteChordDetection(
      double freq1,
      double amp1,
      double freq2,
      double amp2,
    ...
    assertTrue(
        result.getPitchCount() == 4,
        "Should detect 4 pitches in the 4-note chord but was " + result.getPitchCount());
    ...
    int detectedCount =
        (foundFreq1 ? 1 : 0) + (foundFreq2 ? 1 : 0) + (foundFreq3 ? 1 : 0) + (foundFreq4 ? 1 : 0);
    assertTrue(
        detectedCount == 4,
  ...
  /**
   * Generates a chord (multiple sine waves) based on the given frequencies, amplitudes, sample
   * rate, and duration.
   *
  ...
  /**
   * Tests that clear chords (without noise) have high confidence values. This test verifies that
   * the confidence calculation produces high values for clean, well-defined chords with strong
   * harmonic content.
   */
  @Test
  void testClearChordConfidence() {
    ...
    double[] chord =
        generateChord(
            new double[] {261.63, 329.63, 392.0}, new double[] {1.0, 1.0, 1.0}, SAMPLE_RATE, 1.0);
  ...
  /**
   * Tests how confidence decreases as noise level increases in a chord. This test verifies that the
   * confidence calculation is sensitive to the signal-to-noise ratio, with confidence decreasing as
   * noise increases.
   */
  @ParameterizedTest
    ...
    if (result.hasPitches()) {
      ...
      if (noiseLevel <= 0.005) {
        assertTrue(
            ...
            "Confidence should be high (>0.7) for very low noise level "
                + noiseLevel
                + ", but was "
                + result.confidence());
      }
      // For low noise (0.01-0.02), confidence should be moderate to high
        ...
        assertTrue(
            ...
            "Confidence should be moderate to high (>0.6) for low noise level "
                + noiseLevel
                + ", but was "
                + result.confidence());
      }
      // For medium noise (0.05), confidence can be lower
        ...
        assertTrue(
            ...
            "Confidence should be between 0 and 1 for medium noise level "
                + noiseLevel
                + ", but was "
                + result.confidence());
      }
  ...
  /**
   * Tests how confidence varies with the number of notes in a chord. This test verifies that the
   * confidence calculation works well for both single notes and complex chords.
   */
  @ParameterizedTest
  ...
  void testSingleNoteVsChordConfidence(int noteCount) {
    ...
    assertTrue(
        result.confidence() > 0.7,
        "Confidence should be high (>0.7) for "
            + noteCount
            + " clean note(s), but was "
            + result.confidence());
  }

  /**
   * Tests confidence for signals with varying amplitudes. This test verifies that the confidence
   * calculation works correctly for signals with different amplitudes, from very low to high.
   *
   * <p>Note: The current implementation of the chord detector maintains high confidence even for
   * very low amplitude signals, as long as the signal-to-noise ratio is good. This is actually a
   * desirable behavior for many applications.
   */
  @ParameterizedTest
  ...
  void testLowAmplitudeSignalConfidence(double amplitude) {
    ...
    if (amplitude < 0.005) {
      ...
      if (result.hasPitches()) {
        ...
        assertTrue(
            result.confidence() >= 0.0 && result.confidence() <= 1.0,
            "Confidence should be between 0 and 1 for extremely low amplitude "
                + amplitude
                + ", but was "
                + result.confidence());
      }
      ...
      if (amplitude >= 0.5) {
        ...
        assertTrue(
            result.confidence() > 0.7,
            "Confidence should be high (>0.7) for high amplitude "
                + amplitude
                + ", but was "
                + result.confidence());
        ...
        assertTrue(
            result.confidence() > 0.6,
            "Confidence should be good (>0.6) for medium amplitude "
                + amplitude
                + ", but was "
                + result.confidence());
      } else {
        ...
        assertTrue(
            ...
            "Confidence should be reasonable (>0.5) for low amplitude "
                + amplitude
                + ", but was "
                + result.confidence());
      }
