 ...
 *
 * <p>This class provides methods for generating various types of audio signals commonly used in
 * testing pitch detection algorithms, such as sine waves, mixed sine waves, sine waves with noise,
 * frequency sweeps, and signals with multiple frequency components.
 *
 * <p>These utilities help ensure consistent test signal generation across different test classes,
...
public class AudioTestUtils {
   ...
   *
   * <p>This method creates a pure sine wave with amplitude 1.0 at the given frequency. The
   * resulting array contains samples that represent the sine wave over the specified duration at
   ...
   *
   * <p>This method creates a complex waveform by combining two sine waves with different
   * frequencies and amplitudes. This is useful for testing how pitch detection algorithms handle
   ...
   */
  public static double[] generateMixedSineWaveWithAmplitudes(
      double primaryFreq,
      double primaryAmp,
    ...
    for (int i = 0; i < samples; i++) {
      ...
      audioData[i] =
          primaryAmp * Math.sin(2 * Math.PI * primaryFreq * t)
   ...
   *
   * <p>This method creates a sine wave and adds random noise to it. The noise level parameter
   * controls the amplitude of the noise relative to the sine wave. This is useful for testing how
   * pitch detection algorithms perform in the presence of noise.
   *
   * @param frequency the frequency of the sine wave in Hz
   * @param sampleRate the sample rate in Hz
   ...
   */
  public static double[] generateSineWaveWithNoise(
      double frequency, int sampleRate, int durationMs, double noiseLevel) {
   ...
   *
   * <p>This method creates random noise with values between -1 and 1. This is useful for testing
   * how pitch detection algorithms handle non-periodic signals.
   *
   * @param sampleRate the sample rate in Hz
   * @param duration the duration in seconds
   ...
   *
   * <p>This method creates a signal that increases linearly from 0 to 1 over the specified
   * duration. This is useful for testing how pitch detection algorithms handle non-periodic
   * signals.
   *
   * @param sampleRate the sample rate in Hz
   * @param duration the duration in seconds
  ...
  /**
   * Generates a zero-crossing wave (square-like waveform) with the specified frequency, amplitude,
   * sample rate, and duration.
   *
   * <p>This method creates a waveform that alternates between positive and negative amplitude
   * values based on the sign of a sine wave at the given frequency. This is useful for testing how
   * pitch detection algorithms handle signals with sharp transitions.
   *
   ...
   */
  public static double[] generateZeroCrossingWave(
      double frequency, double amplitude, int sampleRate, double duration) {
   ...
   *
   * <p>This method creates a sine wave whose amplitude increases linearly from 0 to 1 over the
   * specified duration. This is useful for testing how pitch detection algorithms handle signals
   ...
   */
  public static double[] generateSineWaveWithVaryingAmplitude(
  ...
  /**
   * Generates a frequency sweep (chirp) signal that transitions from a start frequency to an end
   * frequency over the specified duration.
   *
   * <p>This method creates a signal whose frequency changes linearly from the start frequency to
   * the end frequency. This is useful for testing how pitch detection algorithms track changing
   * frequencies, especially in the high frequency range.
   *
   ...
   */
  public static double[] generateFrequencySweep(
      double startFreq, double endFreq, int sampleRate, double duration) {
    int samples = (int) (sampleRate * duration);
    double[] audioData = new double[samples];
   ...
   *
   * <p>This method creates a complex waveform by combining multiple sine waves with different
   * frequencies and amplitudes. This is useful for testing how pitch detection algorithms handle
   * signals with multiple frequency components, which is common in real-world musical sounds like
   * harmonicas.
   *
   ...
   */
  public static double[] generateMultipleFrequencySignal(
      double[] frequencies, double[] amplitudes, int sampleRate, double duration) {
   ...
   *
   * <p>This method creates a signal that starts at one frequency and then abruptly changes to
   * another frequency at a specified transition point. This is useful for testing how quickly pitch
   * detection algorithms can respond to sudden frequency changes, which is important for real-time
   * applications.
   *
   ...
   * @param transitionPoint the point at which to transition from freq1 to freq2, as a fraction of
   *     duration (0.0-1.0)
   * @return an array of double values representing the signal with a frequency transition
   */
  public static double[] generateFrequencyTransition(
      double freq1, double freq2, int sampleRate, double duration, double transitionPoint) {
    int samples = (int) (sampleRate * duration);
