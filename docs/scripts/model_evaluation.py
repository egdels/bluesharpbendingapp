...
NUM_CLASSES = 12  # 12 semitones in an octave
...
def load_model(model_dir):
    ...
    # Try to load the model from .pb format
        ...
        interpreter = tf.lite.Interpreter(model_path=pb_path)
...
def process_labels(labels_list):
    ...
    for label in labels_list:
        if isinstance(label, dict):
            ...
            if label.startswith(('C', 'D', 'E', 'F', 'G', 'A', 'B')):
                # Handle note labels (e.g., 'C4', 'A3', etc.)
                ...
                # Default to C (0) if label is not a note
                processed_labels.append(0)
...
def evaluate_model(model, X_test, y_test, batch_size=32):
    ...
    # Check if the model is a TensorFlow Lite interpreter
    if isinstance(model, tf.lite.Interpreter):
        ...
        for i in range(0, len(X_test), batch_size):
            ...
            for x in batch_data:
                ...
                if input_shape[0] == 1:  # If the model expects a batch dimension of 1
...
def predict_audio_file(model, audio_path, feature_type='all'):
    ...
    # Map the class index to a note name
    note_names = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']
    predicted_note = note_names[predicted_class]
...
def visualize_confusion_matrix(cm, output_dir):
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=note_names, yticklabels=note_names)
...
def main():
    ...
    if args.audio_file:
        ...
        if y_test is None:
            ...
            # Map the class indices to note names
            note_names = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']
            predicted_notes = [note_names[i] for i in predicted_classes]
